...
const checkClass = (hero, characterClass) => {
	...
	switch(lowerCharacterClass) {
		...
		case `rogue`:
			...
			break;
		default:
			characterClass = prompt(`"${characterClass}" is not a valid class. Please choose between Warrior, Ranger and Rogue.`);
...
const checkRace = (hero, race) => {
	...
	switch(lowerCaseRace) {
		...
		case `halfling`:
			...
			break;
		default:
		race = prompt(`${race} is not a valid race. Please choose between Human, Elf, Dwarf and Halfling.`);
		hero.race = race;
		checkRace(hero, race);
		break;
	}
